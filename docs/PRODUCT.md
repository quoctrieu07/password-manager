...
| :--- | :--- | :--- | :--- | :--- |
| P1-01 | Khung UI 3 cột | Dựng giao diện thuần theo cảm hứng KeeWeb | \- Hiển thị đủ Sidebar, Danh sách và Panel chi tiết.\- Giao diện Responsive trên PC bằng CSS thuần. | Bắt buộc |
| P1-02 | Đọc/Ghi file JSON | Tạo mới, import và xuất tệp dữ liệu cục bộ | \- Kéo thả file .json để tải dữ liệu lên web.\- Bấm “Save” kích hoạt tải file .json mới về máy. | Bắt buộc |
| P1-03 | CRUD tài khoản | Xử lý các thao tác tương tác với mật khẩu | \- Thêm, sửa, xóa thành công một mục tài khoản.\- Click chọn mục nào hiển thị đúng chi tiết mục đó. | Bắt buộc |
| P1-04 | Tiện ích Mật khẩu | Bộ công cụ hỗ trợ xử lý chuỗi mật khẩu | \- Tự sinh mật khẩu mạnh ngẫu nhiên theo yêu cầu.\- Có nút ẩn/hiện (\*) và nút sao chép nhanh. | Bắt buộc |
| P1-05 | Bộ đếm mã OTP | Tự động sinh mã 2FA từ khóa Secret | \- Hiển thị chính xác mã OTP 6 số theo thời gian thực.\- Tự làm mới mã sau mỗi 30 giây. | Bắt buộc |
| P1-06 | Đa ngôn ngữ & Lịch sử | Tích hợp i18n và bảng nhật ký hoạt động | \- Đổi ngôn ngữ (Anh/Việt) không cần tải lại trang.\- Bảng History hiện đúng nhật ký thao tác vừa làm. | Bắt buộc |
| P2-01 | Mã hóa file AES | Bảo vệ file JSON bằng Master Password | \- File tải về được mã hóa thành chuỗi ký tự vô nghĩa.\- Yêu cầu nhập đúng Master Password để giải mã. | Bắt buộc |
| P2-02 | Tự động khóa | Tự khóa màn hình khi người dùng rời máy | \- Sau 5 phút không hoạt động chuột/phím, RAM tự xóa dữ liệu.\- Hiển thị màn hình khóa yêu cầu nhập lại mật khẩu. | Có thể có |
| P2-03 | Theme Material You | Nâng cấp thẩm mỹ giao diện động hiện đại | \- Chuyển đổi mượt mà chế độ Sáng/Tối.\- Màu sắc giao diện thay đổi đồng bộ theo hệ thống. | Nên có |
| P2-04 | Kéo thả sắp xếp | Tối ưu hóa trải nghiệm điều hướng danh mục | \- Dùng chuột kéo thả tài khoản vào các nhóm ở Sidebar.\- Bảng Settings cho phép chỉnh thời gian tự khóa linh hoạt. | Nên có |
| P3-01 | Hệ thống Server Sync | Xây dựng máy chủ đồng bộ dữ liệu tập trung | \- Triển khai API đăng ký, đăng nhập và phân quyền trên SQLite.\- Hỗ trợ upload/download các tệp vault đã mã hóa lên máy chủ. | Bắt buộc |
| P3-02 | Bảo mật đường truyền | [DA phụ trách] Mã hóa gói tin và xác thực an toàn | \- Băm mật khẩu trước khi gửi lên mạng.\- Áp dụng mã hóa phiên và token JWT để chặn đánh chặn dữ liệu. | Có thể có |
| P3-03 | Responsive Mobile UI | Tối ưu hóa layout thích ứng với màn hình nhỏ | \- Tự động ẩn Sidebar thành menu trượt khi viewport thu nhỏ.\- Sắp xếp lại danh sách entry theo chiều dọc phù hợp với di động. | Bắt buộc |
| P3-04 | Gọt giũa cấu trúc GUI | Loại bỏ cấu phần thừa và tối ưu hóa điều hướng | \- Loại bỏ một số nút thừa, thay đổi GUI để cùng lúc phù hợp với nhiều thiết bị khác nhau. | Bắt buộc |
